...
import brock.api.utils.htmx.descriptors.attributes.*
import brock.api.utils.htmx.descriptors.attributes.httpMethods.*
...
class HtmxMethods(private val map: MutableMap<String, String>) {
    ...
    fun get(url: String, lambda: HxGet.() -> Unit = {}) = addEntry(HxGet(url), lambda)

    fun post(url: String, lambda: HxPost.() -> Unit = {}) = addEntry(HxPost(url), lambda)

    fun put(url: String, lambda: HxPut.() -> Unit = {}) = addEntry(HxPut(url), lambda)

    fun delete(url: String, lambda: HxDelete.() -> Unit = {}) = addEntry(HxDelete(url), lambda)

    fun patch(url: String, lambda: HxPatch.() -> Unit = {}) = addEntry(HxPatch(url), lambda)
    ...
    fun preserve(option: String = "", lambda: HxPreserve.() -> Unit = {}) = addEntry(HxPreserve(option), lambda)

    fun params(lambda: HxParams.() -> Unit) = addEntry(HxParams(), lambda)

    fun encoding(encodingOptions: HxEncodingOptions, lambda: HxEncoding.() -> Unit = {}) =
        addEntry(HxEncoding(encodingOptions), lambda)

    fun vals(lambda: HxVals.() -> Unit) = addEntry(HxVals(), lambda)

    fun confirm(message: String, lambda: HxConfirm.() -> Unit = {}) = addEntry(HxConfirm(message), lambda)
    ...
    fun boost(enabled: Boolean, lambda: HxBoost.() -> Unit = {}) = addEntry(HxBoost(enabled), lambda)
    ...
    fun history(enabled: Boolean, lambda: HxHistory.() -> Unit = {}) = addEntry(HxHistory(enabled), lambda)

    fun validate(lambda: HxValidate.() -> Unit = {}) = addEntry(HxValidate(), lambda)

    fun on(event: String? = null, htmxEvent: String? = null, lambda: HxOn.() -> Unit) =
    ...
    private fun <T : HxDescriptor> addEntry(instance: T, lambda: T.() -> Unit) {
